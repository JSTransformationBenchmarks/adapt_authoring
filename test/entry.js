(async function () {
  ...
  before(function (done) {
    ...
    async.series([removeTestData, createCacheData, function startApp(cb) {
      logger.level('console', 'error'); // only show errors

      app.use({
        configFile: path.join('test', 'testConfig.json')
      });
  ...
  function createTestTenant(tenantDetails, cb) {
    tenantmanager.createTenant(tenantDetails, function (error, tenant) {
      if (error && error instanceof tenantmanager.errors.DuplicateTenantError) {
        return tenantmanager.retrieveTenant({
  ...
  function removeTestData(done) {
    async.parallel([function dumpOldDb(cb) {
  ...
  function createCacheData(done) {
    ...
    function _copyFramework() {
      ...
      async.series([async.apply(fs.remove, dest), async.apply(fs.ensureDir, dest), async.apply(fs.symlink, src, path.join(dest, Folders.Framework), 'junction')], done);
  ...
  async function testLoader() {
    var contents = await fs_readdirPromise(__dirname);
